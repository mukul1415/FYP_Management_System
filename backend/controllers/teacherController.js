...
export const acceptRequest = asyncHandler(async (req, res, next) => {
  ...
  await sendEmail({
    to: studentEmail,
    subject: "FYP SYSTEM - ✅ Your Supervisor R}equest has been accepted",
    message,
  });
...
export const rejectRequest = asyncHandler(async (req, res, next) => {
  ...
  await sendEmail({
    to: studentEmail,
    subject: "FYP SYSTEM - ❌ Your Supervisor R}equest has been rejected",
    message,
  });
...
export const getAssignedStudents = asyncHandler(async (req, res, next) => {
  ...
  res.status(200).json({
    ...
    data: {
      ...
      total: studentsWithProjects.length,
    },
  });
});
...
export const addFeedback = asyncHandler(async (req, res, next) => {
  ...
  await notificationServices.notifyUser(
    ...
    "feedback",
    "/students/feedback",
    type === "pisitive" ? "low" : type === "negative" ? "high" : "low",
  );
